# Ques : Print the HackerRank logo using string alignment methods (.ljust, .center, .rjust) for a given thickness.
...
for i in range((thickness+1)//2): print((c*thickness).center(thickness*6))
for i in range(thickness+1): print((c*thickness).center(thickness*2)+(c*thickness).center(thickness*6))
